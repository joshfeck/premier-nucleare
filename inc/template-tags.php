...
function nucleare_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			<div class="nav-previous"><?php next_posts_link( '<div class="meta-nav"><i class="fa fa-lg fa-angle-left spaceRight"></i><span class="smallPart">'. __('Older Posts', 'nucleare') .'</span></div>' ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( '<div class="meta-nav"><span class="smallPart">'. __('Newer Posts', 'nucleare') .'</span><i class="fa fa-lg fa-angle-right spaceLeft"></i></div>' ); ?></div>
			<?php endif; ?>
...
function nucleare_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', '<div class="theNavigationArrow"><i class="fa prevNext fa-2x fa-angle-left"></i></div><div class="meta-nav" aria-hidden="true"><span class="smallPart">' . __( 'Previous Post', 'nucleare' ) . '</span> ' . '<span class="screen-reader-text">' . __( 'Previous post:', 'nucleare' ) . '</span> ' . '<div class="nextPrevName">%title</div></div>' );
				next_post_link( '<div class="nav-next">%link</div>', '<div class="meta-nav" aria-hidden="true"><span class="smallPart">' . __( 'Next Post', 'nucleare' ) . '</span><div class="nextPrevName">%title</div></div><div class="theNavigationArrow"><i class="fa prevNext fa-2x fa-angle-right"></i></div> ' . '<span class="screen-reader-text">' . __( 'Next Post:', 'nucleare' ) . '</span> ');
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function nucleare_posted_on() {
	...
	$posted_on = sprintf(
		_x( '<i class="fa fa-clock-o spaceLeftRight"></i>%s', 'post date', 'nucleare' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( '<i class="fa fa-user spaceLeftRight"></i>%s', 'post author', 'nucleare' ),
...
function nucleare_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {		
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( '<i class="fa fa-tags spaceRight"></i>%1$s', 'nucleare' ) . '</span>', $tags_list );
		}
	}

	edit_post_link( __( 'Edit', 'nucleare' ), '<span class="edit-link"><i class="fa fa-wrench spaceRight"></i>', '</span>' );
}
endif;
